...
function validateDate(m,d,y)
{
	...
	if( d > MonthLengths[m] ){
		alert(Months[m] + " no contiene " + d + " días.");
		return false;
	}
	//negative days
	if(d < 1){
		alert('Días debe ser un número positivo.');
		return false;
	}
	
	//year == 0
	if(y == 0){
		alert("Calendario juliano no tiene un año cero.");
		return false;
	}
	
	//keep valies between -4000 and 4000
	if(y > 4000 || y < -4000){
		alert("Por favor introduzca un año entre -4000 (B.C.) and 4000 (A.D.)");
		return false;
	}
	
	if ((m== 1) && (d == 29)){
    if (y % 4 != 0){
			alert(Months[m] + " no contiene " + d + " Día en el año " + y);
			return false;
		}
